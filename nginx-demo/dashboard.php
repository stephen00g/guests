...
function nodeCpuType($node) {
  static $intelFamily6Model = [
    26 => 'Nehalem', 30 => 'Nehalem', 37 => 'Westmere', 42 => 'Sandy Bridge', 45 => 'Sandy Bridge',
    58 => 'Ivy Bridge', 60 => 'Ivy Bridge', 63 => 'Haswell', 69 => 'Haswell', 79 => 'Broadwell',
    85 => 'Skylake', 94 => 'Skylake', 142 => 'Kaby Lake', 158 => 'Coffee Lake', 165 => 'Comet Lake',
    166 => 'Ice Lake', 167 => 'Rocket Lake', 183 => 'Alder Lake', 190 => 'Raptor Lake',
  ];
  ...
  if ($vendor !== '' || $family !== '' || $modelId !== '') {
    ...
    if ($family !== '' && $family !== '6') $parts[] = 'Family ' . $family;
    if ($modelId !== '') {
      $mid = (int)$modelId;
      $fam = trim((string)$family);
      $ven = trim((string)$vendor);
      $isIntelF6 = ($fam === '6' || $fam === '') && (strcasecmp($ven, 'GenuineIntel') === 0 || stripos($ven, 'intel') !== false);
      $codename = ($isIntelF6 && isset($intelFamily6Model[$mid])) ? $intelFamily6Model[$mid] : null;
      $parts[] = 'Model ' . $modelId . ($codename !== null ? ' (' . $codename . ')' : '');
    } elseif ($family !== '') {
      $parts[] = 'Family ' . $family;
    }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .update-banner button:hover { background: #6ba3ff; }
    ...
    .namespace-list .row { padding: 0.5rem 0; border-bottom: 1px solid #252526; }
...
<body>
  ...
  <div class="container">
    ...
    <?php else: ?>
      ...
      <div class="panel">
        <div class="panel-header">Nodes <span class="header-tip" title="CPU type shows vendor, family and model when Node Feature Discovery (NFD) is installed; otherwise architecture or cloud instance type." aria-label="Info">ⓘ</span></div>
  ...
  (function() {
    ...
    checkVersion();
    setInterval(checkVersion, versionCheckInterval);
